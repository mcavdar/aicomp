...
class NearDuplicatesIndex:
    ...
    def get_jaccard2(self, docname1, doc):

        p = self.p
        ngrams = self.calculate_ngrams(doc, 3)
        tmpSketch = self.calculate_sketch("zz", ngrams)

        if not (self.sketches[docname1] or tmpSketch):
            raise Exception

        # get num of same sketch values
        k = 0.0
        for index in xrange(self.n):
            if self.sketches[docname1][index] == tmpSketch[index]:
                k += 1
        return self.jaccard(k)
